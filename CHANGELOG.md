...
### Fixed
- **Android: Foto-Vorschau im Bearbeiten-Formular kaputt, wenn das Foto nur beim Sync-Anbieter liegt**: `PhotoPickerField` (Lack-, Sticker- und Tagebuch-Formular) rief `resolvePhotoUri()` bedingungslos auf, was immer eine lokale `file://`-URI baut — für ein Foto, das auf einem anderen Gerät aufgenommen und nur synchronisiert (nicht heruntergeladen) wurde, existiert diese Datei aber nie lokal. Die Kartenansicht zeigte das Foto korrekt (über `rememberPhotoModel`/`PhotoResolution`), das Bearbeiten-Formular blieb leer. `PhotoPickerField` prüft jetzt per `PhotoRepository.existsLocally()`, ob die Datei lokal vorliegt, und weicht sonst auf denselben Remote-Auflösungspfad wie die Kartenansicht aus. (#163)
