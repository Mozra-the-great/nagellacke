...
### Behoben
- **Android erklärt jetzt, wenn der Server Foto-Uploads oder KI abgeschaltet hat.** Beim ZIP-Import landeten vom Server abgelehnte Fotos im allgemeinen Hinweis „Sync konfiguriert?"; jetzt heißt es, dass der Server keine Foto-Uploads annimmt, und nach der ersten Ablehnung werden die übrigen Fotos gar nicht erst versucht. Das Ergebnis von KI-Auto-Fill wurde bisher verworfen, sodass ein am Server abgeschalteter oder erfolgloser Auftrag aussah wie einer, der noch läuft; Sammlung und Wunschliste zeigen es jetzt als kurze Meldung an, im Fehlerfall mit dem Grund vom Server. (#324, Schritt S22)
- **Nach einem Neuladen verschwand der Admin-Tab.** Die Rollenabfrage lief beim Start, bevor die Web-App das Refresh-Cookie gegen ein Access-Token getauscht hatte (#299). Die Sitzung überlebte den Reload, der Admin-Tab nicht. Die Rolle wird jetzt nach der Wiederherstellung der Sitzung erneut abgefragt.
- **Android: Berichte und KI-Funktionen erneuerten ein abgelaufenes Token nicht.** `ReportsClient` und `AiClient` schickten das beim Erstellen gelesene Token fest mit, und nach Ablauf des Access-Tokens scheiterte jeder Aufruf mit 401, bis zufällig ein Sync das Token erneuerte. Beide laufen jetzt über `ServerSession`: Bei einem 401 wird einmal erneuert und einmal wiederholt, wie beim Sync. (#349)
- **Kaum sichtbarer Tastatur-Fokus in der Web-App.** Außer der Navigation und den Textfeldern zeigte kein Bedienelement einen erkennbaren Fokusrahmen; der Browser-Standard hatte vor dem dunklen Hintergrund einen Kontrast von etwa 1,05:1. Alle Buttons, Links und fokussierbaren Elemente bekommen jetzt den 2-px-Rahmen der Navigation (Kontrast etwa 11:1). (#346)
- **Barrierefreiheit:** Das Smart-Cart-Eingabefeld und das Feld „Neue Kategorie" in der Web-App haben jetzt einen zugänglichen Namen (#357), das Smart-Cart-Feld unter Android ein bleibendes Label (#358). Die beiden Datumsfelder unter Android (Tagebuch, Berichte) waren als deaktiviert markiert, und TalkBack bietet deaktivierte Elemente nicht zur Bedienung an. Sie bleiben jetzt aktiv und schreibgeschützt und haben einen beschrifteten Knopf, der die Datumsauswahl öffnet. (#359)
- Android: Das Tagebuch filterte Wunschlisten-Lacke über einen Textvergleich mit dem Enum-Namen statt über die Enum-Konstante selbst. (#350)
- **Android löschte Fotos auf dem Sync-Ziel nie.** Alle fünf Adapter implementieren `deletePhoto()`, aufgerufen wurde es nirgends — weder beim Löschen eines Lacks, Stickers oder einer Maniküre noch beim Ersetzen eines Fotos. Die App merkt sich solche Dateinamen jetzt und löscht sie nach dem nächsten erfolgreichen Sync auf Server, Nextcloud, Google Drive, OneDrive oder Dropbox — erst nachdem die Löschung selbst synchronisiert ist, und nur, wenn kein lebender Datensatz das Foto nach dem Abgleich wieder verwendet. Ein fehlgeschlagener Löschversuch bricht den Sync nicht ab und wird bis zu dreimal wiederholt. (#348)
- **Beim Löschen eines Lacks, Stickers oder einer Maniküre blieb das Foto auf dem Server liegen.** Der Sync-Adapter schickte bei jedem Aufruf `Content-Type: application/json`, auch beim `DELETE /api/photos/…` ohne Body, und Fastify lehnt einen leeren JSON-Body mit 400 ab. Die Web-App verschluckte den Fehler, weil sie das Löschen als „best effort" behandelt, und der Adapter prüfte die Antwort gar nicht erst. Der Header fehlt jetzt bei Anfragen ohne Body, ein abgelehntes Löschen wirft einen Fehler, und die Web-App protokolliert ihn in der Konsole. (#344)
- **„Abmelden" meldete nicht wirklich ab.** Der Knopf in Einstellungen → Sync räumte nur den Zustand der Seite auf: Der Admin-Tab samt zuvor geladener Werte blieb sichtbar, bis man die Seite neu lud, und das httpOnly-Refresh-Cookie aus #299 blieb für seine vollen 30 Tage gültig im Browser, denn ein Skript kann es nicht löschen. Der neue Endpunkt `POST /api/auth/logout` löscht das Cookie für dieses Gerät, die Web-App ruft ihn beim Abmelden auf, fragt die Rolle neu ab und verwirft den zwischengespeicherten Foto-Token. Andere Geräte bleiben angemeldet; dafür gibt es weiterhin `logout-all`. (#345)
- **Der Server setzte keinerlei Sicherheits-Header.** Die Web-App, die eine install.sh-Installation selbst ausliefert, ließ sich von jeder fremden Seite in einen `<iframe>` einbetten (Clickjacking). Jede Antwort trägt jetzt `X-Frame-Options: DENY`, `Content-Security-Policy: frame-ancestors 'none'; base-uri 'self'; object-src 'none'`, `X-Content-Type-Options: nosniff` und `Referrer-Policy: no-referrer`. HSTS setzt der Server bewusst nicht, das ist Sache des TLS-Proxys davor. (#355)
- **Jeder angemeldete Account konnte die Fotos jedes anderen abrufen.** `GET /photos/:filename` prüfte nur, *ob* ein gültiges Bearer-JWT oder ein gültiger Foto-Token vorlag, nicht, wem das Foto gehört — und einen Sitzungs-Token (`GET /api/photos/token`) kann sich jeder Account selbst ausstellen. Wer einen fremden Dateinamen kannte, bekam das Bild. Zugleich reichte die Besitzprüfung aus #343 nicht: sie fragte nur, ob ein eigener Datensatz den Dateinamen referenziert, und einen solchen Datensatz kann jeder Account per Sync anlegen. Der Server merkt sich jetzt beim Upload, wer ein Foto hochgeladen hat (`data/photo_owners.json`, beim ersten Start aus den vorhandenen Datensätzen befüllt), und genau das entscheidet über Lesen, Löschen und die signierten Bild-Links in Berichts-Mails. Referenzen zählen nur noch bei Fotos ohne bekannten Uploader (per `X-Api-Key` hochgeladen). Admins und der `X-Api-Key` erreichen weiterhin jedes Foto. Nebeneffekt: ein hochgeladenes, noch nirgends verwendetes Foto kann sein Uploader jetzt selbst löschen. (#352)
- **Der Berichtsplan war eine einzige Einstellung für den ganzen Server.** Jeder angemeldete Account konnte über `GET /api/reports/schedule` die Berichts-E-Mail-Adresse eines anderen lesen und sie mit `POST` überschreiben, und Einstellungen → Sync zeigte die Adresse des Vorgängers vorausgefüllt an. `schedule.json` hält jetzt einen Plan pro Account, beide Routen sehen nur den eigenen, und der stündliche Versand arbeitet jeden Plan für sich ab. Ein vorhandener Plan wird beim Start dem Account zugeordnet, der ihn zuletzt gespeichert hat. (#353)
- **`DELETE /api/photos/:filename` hatte gar keine Besitzprüfung.** Jeder authentifizierte Account — admin oder nicht — konnte das Foto eines beliebigen anderen Nutzers löschen, sofern er den (zufällig generierten, aber nicht anderweitig geschützten) Dateinamen kannte; verifiziert mit zwei unabhängigen Accounts, bei denen `bob` `alice`s Foto per `DELETE` entfernen konnte. Es gibt keine eigene Besitz-Tabelle für Fotos — der Endpunkt prüft jetzt stattdessen (`userOwnsPhoto()` in `db.ts`), ob der angeforderte Dateiname in einem der eigenen Datensätze des aufrufenden Nutzers (Lack, Sticker, Maniküre — inklusive weich gelöschter, da der Undo-Snackbar-Flow des Web-Apps `deletedAt` setzt und synct, *bevor* er den Foto-Löschaufruf macht) tatsächlich referenziert wird, und verlangt sonst `403`. Admins dürfen weiterhin jedes Foto löschen; der `X-Api-Key`-Pfad bleibt unverändert (bereits ein root-artiges Credential, wie beim Upload). (#343)
- **Das Selbst-Update legte den kompletten Server lahm, solange es lief.** `runUpdate()` trieb seine Schritte mit `spawnSync` an, was den Node-Event-Loop für die gesamte Dauer blockiert — der Server konnte in dieser Zeit *keine* Anfrage beantworten, auch nicht das `GET /api/update/status`, das in #335 genau dafür eingeführt wurde, dass die Oberfläche zusehen kann. Auf der Test-Instanz gemessen: 30,5 s zwischen `startedAt` und `finishedAt`, und kein einziger Poll in diesem Fenster bekam eine Antwort. Die Statusdatei war bei jedem Schritt korrekt geschrieben — nur lesen konnte sie niemand. Die Schrittanzeige (`Update läuft — Schritt 4/8: Paket core bauen`) war damit nicht falsch, sondern unerreichbar: in der Praxis stand die ganze Zeit der Platzhalter „Update gestartet…", gefolgt vom Endzustand. Die Schritte laufen jetzt über das asynchrone `child_process.spawn`, `runUpdate()` ist `async`; der Event-Loop dreht weiter und die Polls landen zwischen den Schritten. Der zweite, größere Effekt war ohnehin nicht die Anzeige: der gesamte Server war für die Dauer des Updates weg, auf der Test-Box 30 s, auf einer langsameren Maschine mit kaltem `node_modules` eher Minuten statt der wenigen Sekunden, die der Neustart allein kostet. Zusätzlich protokolliert jeder Schritt seinen Start ins Journal, damit ein Betreiber ohne Admin-Oberfläche mitlesen kann — die Statusdatei ist admin-geschützt. Ein Schritt, der sein Timeout überschreitet, wird jetzt hart beendet und meldet das als Grund, statt als exit-code-loser Fehlschlag ohne `stderr` zu erscheinen. (#338)
...
### Hinzugefügt
- **Serverweite Schalter für Foto-Uploads und KI-Funktionen** unter Admin → Server-Einstellungen, dazu der Schalter „Öffentliche Instanz". Beide Funktionsschalter werden serverseitig durchgesetzt, nicht nur in der Oberfläche ausgeblendet: Ist der Foto-Schalter aus, lehnt `POST /api/photos` neue Uploads mit 403 ab, auch per API-Schlüssel; vorhandene Fotos bleiben sichtbar und löschbar. Ist der KI-Schalter aus, lehnen die KI-Endpunkte neue Aufträge mit 403 ab, und die Warteschlange hält bereits eingereihte an, statt Anbieter-Kontingent zu verbrauchen. Der neue öffentliche Endpunkt `GET /api/instance-config` meldet die Schalter an die Web-App. Die Web-App liest den Endpunkt beim Start und nach jeder Anmeldung. Sie blendet Auto-Fill, Smart-Cart und die KI-Einstellungen aus, wenn der Server KI abgeschaltet hat, und zeigt im Foto-Feld einen Hinweis statt des Upload-Knopfs, wenn Uploads gesperrt sind. Auf einer öffentlichen Instanz entfällt der Knopf „Jetzt syncen". Ein älterer Server ohne diesen Endpunkt ändert an der Oberfläche nichts. Rechtstexte und die Einleitungsseite des öffentlichen Modus folgen in späteren Schritten. (#324, Schritte S1–S5, S7, S10)
- **Branding unter Admin → Branding:** Wählbar sind die Vorlagen „Nail Lacquer" (wie bisher) und „NailVault" mit einem Platzhalter-Schriftzug oder ein eigenes Branding mit Name, Untertitel, Akzentfarbe, Logo (PNG, JPEG oder WebP) und Einleitungstext, samt Vorschau. Name und Logo erscheinen im Kopf der Web-App, der Titel im Browser-Tab und im Bericht; die Akzentfarbe setzt nur die Primärfarbe. Gespeichert wird in `data/branding.json`, getrennt von den Server-Einstellungen, weil sie öffentlich ausgeliefert wird. Ein Logo prüft der Server an seinen eigenen Dateisignaturen; SVG wird abgelehnt, weil es auf dieser Origin Skript ausführen könnte. (#324, Schritte S6, S8, S9)
- **Impressum und Datenschutzerklärung pro Instanz** unter Admin → Rechtstexte, als reiner Text: Leerzeilen trennen Absätze, Web- und E-Mail-Adressen werden verlinkt. Die Seiten sind unter `#/impressum` und `#/datenschutz` direkt verlinkbar und ohne Anmeldung erreichbar. Ein gepflegter Text bekommt einen Link in der Fußzeile; ohne Rechtstexte sieht die App aus wie bisher. Die Texte liegen in `data/legal.json` und werden nie als HTML gerendert. (#324, Schritte S11, S12)
- **Sicherheitsprüfung bei der Registrierung** (Admin → Server-Einstellungen, standardmäßig aus): Vor dem Anlegen eines Kontos rechnet das Gerät eine kleine Rechenaufgabe, die der Server ausgibt und signiert. Im Browser dauert das weniger als eine Sekunde und läuft in einem Web Worker, damit die Seite bedienbar bleibt. Für massenhaft angelegte Konten wird es dagegen teuer. Es gibt keinen Captcha-Anbieter, keine Anfrage an Dritte und keine personenbezogenen Daten; jede gelöste Aufgabe gilt genau einmal und eine Minute lang. Die Android-App löst die Aufgabe ebenfalls, damit sie mit eingeschaltetem Schalter weiter registrieren kann. Unter dem Registrierformular der Web-App stehen jetzt Links zu Impressum und Datenschutz, sofern gepflegt. Das allererste Konto eines Servers ist ausgenommen. (#324, Schritte S13, S14)
- **Einleitungsseite auf öffentlichen Instanzen.** Wer die Web-App einer öffentlichen Instanz ohne Anmeldung öffnet, sieht zuerst den Einleitungstext aus Admin → Branding (ohne gepflegten Text eine allgemeine Beschreibung der App) und wählt zwischen zwei Wegen. Der erste ist Anmelden oder Konto erstellen, im selben Formular wie unter Einstellungen → Sync. Der zweite ist, ohne Konto nur im Browser weiterzuarbeiten; die Seite sagt vorher, was das kostet. Ein Hinweisstreifen hält diese Wahl umkehrbar. Legt jemand danach ein Konto an oder meldet sich an, während im Browser schon Einträge liegen, fragt die App vor dem ersten Sync, ob die lokale Sammlung ins Konto übernommen oder verworfen werden soll. Bis zur Antwort synchronisiert nichts, auch nicht nach einem Neuladen. Auf einer privaten Installation ändert sich nichts. (#324, Schritte S15, S16)
- **Passwort vergessen?** Im Anmeldeformular lässt sich ein Link zum Zurücksetzen per Mail anfordern, mit Benutzername oder E-Mail-Adresse. Der Link gilt eine Stunde und nur einmal. Das neue Passwort beendet alle bisherigen Sitzungen und Foto-Links des Kontos und hebt eine Anmeldesperre nach Fehlversuchen auf. Die Antwort ist immer dieselbe, ob es das Konto gibt oder nicht. Angeboten wird das nur, wenn der Server Mails versenden kann und eine App-URL kennt; der Server speichert nur einen Hash des Tokens. (#324, Schritte S17, S18)
- **Konto-E-Mail und Bestätigung.** Bei der Registrierung lässt sich optional eine E-Mail-Adresse angeben, und unter Einstellungen → Sync lässt sie sich jetzt überhaupt setzen und ändern; bisher gab es dafür nur den Endpunkt, keine Oberfläche. Eine neue Adresse bekommt einen Bestätigungslink (24 Stunden gültig, „Bestätigungsmail erneut senden" einmal pro Stunde). Nur eine bestätigte Adresse kann ein Passwort zurücksetzen; sonst funktionieren Konten ohne Bestätigung vollständig. (#324, Schritt S19)
- **Eigene Daten herunterladen und Konto selbst löschen** unter Einstellungen → Sync. „Meine Kontodaten herunterladen" liefert alles, was der Server über das Konto speichert, als JSON: Kontodaten, Sammlung und Berichtsplan, ohne Passwort-Hash oder andere Geheimnisse (`GET /api/me/export`). „Konto löschen…" verlangt das Passwort und das Eintippen des Benutzernamens und löscht dann endgültig: Sammlung, Berichtsplan, KI-Aufträge und die Fotos, die dieses Konto hochgeladen hat und die kein anderes Konto verwendet (`POST /api/me/delete`). Optional wird auch die Kopie im Browser geleert. Der letzte Admin kann sich nicht löschen, solange es andere Konten gibt; die Admin-Löschung bleibt wie bisher wiederherstellbar. (#324, Schritte S20, S21)
- **Passkeys als dritter Anmeldeweg** neben Passwort und 2FA, freiwillig. Unter Einstellungen → Sicherheit lassen sich Passkeys mit Namen anlegen, auflisten und einzeln entfernen; im Anmeldeformular steht dann „Mit Passkey anmelden", ohne Benutzernamen. Ein Passkey verlangt Fingerabdruck, Gesicht oder Geräte-PIN und ersetzt damit auch den 2FA-Code. Das Passwort bleibt immer gültig, auch ohne Passkeys. Die Domain, an die Passkeys gebunden sind, ergibt sich aus der App-URL und lässt sich unter Admin → Server-Einstellungen auf eine übergeordnete Domain erweitern; das Panel warnt, dass ein Domainwechsel alle bestehenden Passkeys unbrauchbar macht, und entfernt veraltete auf Knopfdruck. Passkeys brauchen HTTPS (oder localhost): Eine Installation unter `http://192.168.…` blendet den Bereich aus und nennt den Grund. Die Android-App bleibt vorerst bei Passwort und 2FA. Neue Server-Abhängigkeit: `@simplewebauthn/server`. (#228)
- **„Von allen Geräten abmelden"** in Einstellungen → Sync. Beendet alle Sitzungen des Kontos und macht alle bisher erzeugten Foto-Links ungültig; der Endpunkt `POST /api/auth/logout-all` war bisher nur per API erreichbar. (#347)
- **Server-Log im Admin-Bereich.** Lädt auf Knopfdruck die letzten 100, 200 oder 500 Zeilen des Journals über `GET /api/logs`. (#356)

### Geändert
- **Die App-URL lässt sich jetzt im Admin-Panel setzen und wirkt sofort.** Bisher las der Server `APP_URL` nur beim Start und zeigte den Panel-Wert bloß an. Jetzt gilt der Panel-Wert, sonst `APP_URL`, bei jedem Mailversand ohne Neustart. Er wird geprüft: nur http(s), ohne Query und Fragment. (#324, Schritt S17)
- **Breite Bildschirme:** Die Web-App wächst ab 1600 px und 2200 px Fensterbreite in Stufen mit (bis 1900 px statt fest 1100 px), die Raster für Lacke, Sticker, Tagebuch und Einkaufswagen bekommen dadurch mehr Spalten. Einstellungen und Admin bleiben als Formulare bei ihrer bisherigen Breite. (#323) Die Landing-Page (`docs/`) wächst ebenso mit und vergrößert auf sehr breiten Bildschirmen die Schrift. (#322)
...
### Behoben
- **Das Selbst-Update lief auf keiner systemd-Installation durch — und meldete trotzdem Erfolg.** Drei voneinander unabhängige Fehler, von denen jeder einzelne genügte, und die alle in einer Entwickler-Shell unsichtbar blieben, weil dort jeder Schritt sauber durchläuft. Erstens brach `git pull origin main` sofort ab: `install.sh` und der Ansible-Deploy führen beide `npm install` im Checkout aus, das schreibt `package-lock.json` um, und ein Pull verweigert danach dauerhaft die Arbeit („Your local changes would be overwritten by merge"). Ein Deployment-Verzeichnis ist keine Arbeitskopie, der Abgleich läuft jetzt über `git fetch` + `git reset --hard FETCH_HEAD` — nur nachverfolgte Dateien, das Datenverzeichnis ist ohnehin gitignoriert. Zweitens setzt die systemd-Unit `NODE_ENV=production`, npm liest das als `--omit=dev` und *entfernt* dabei `tsup`/`vite`/`typescript` — exakt die Binaries, die die folgenden vier Build-Schritte aufrufen. Auf dem Server stand dazu ein einziges `sh: 1: tsup: not found` im Journal, zurück blieben neue Quellen, keine Toolchain und das alte `dist/` im Betrieb. Die Kindprozesse laufen jetzt ohne `NODE_ENV`, und `npm install` bekommt zusätzlich `--include=dev`. Drittens antwortet `POST /api/update/apply` mit `{ ok: true }`, *bevor* der erste Schritt läuft — jeder Fehlschlag sah in der Oberfläche aus wie ein gestartetes Update. Jeder Schritt wird jetzt in `data/update_state.json` protokolliert und über den neuen, admin-geschützten Endpunkt **`GET /api/update/status`** ausgeliefert; beide Oberflächen pollen ihn und zeigen am Ende die neue Version oder den gescheiterten Schritt samt `stderr`. Einschränkung: die Schritte laufen weiterhin über `spawnSync` und blockieren damit den Event-Loop, sodass der Endpunkt *während* eines Updates nicht antwortet — die Anzeige des gerade laufenden Schritts erscheint deshalb praktisch nie, das Ergebnis dagegen zuverlässig (#338). Weil die Statusdatei im `DATA_DIR` liegt, überlebt sie den absichtlichen Neustart am Ende eines erfolgreichen Updates. Ein `running`-Zustand, der älter als 20 Minuten ist, wird als fehlgeschlagen gemeldet statt endlos weiterzudrehen — ein vom OOM-Killer beendeter Build schreibt nie einen Endzustand. (#335)
- **Einstellungen → Sync prüfte das Ergebnis des Update-Aufrufs überhaupt nicht.** Ein abgelehnter API-Schlüssel oder ein ausgeschöpftes Rate-Limit wurde dort als „Update gestartet" angezeigt, weil `res.ok` nie abgefragt wurde. (#335)
- **Ein Update, das scheiterte, während der Admin-Tab geschlossen war, hinterließ keine Spur.** Die Oberfläche übernahm beim Laden nur einen *laufenden* Lauf; der Poll, der Fehler behandelt, startet in dem Fall nie. Der letzte fehlgeschlagene Versuch wird jetzt beim Öffnen der Admin-Seite angezeigt und bleibt stehen, bis der nächste Versuch ihn überschreibt. (#335)

### Geändert
- **Der Web-Build wird beim Update über ein Staging-Verzeichnis ausgetauscht** statt `public/` zuerst zu löschen und dann hineinzukopieren — ein Kopiervorgang, der auf halber Strecke abbrach (volle Platte), ließ die Web-App vorher mit 404 zurück. Fehlt der Build ganz, bricht das Update jetzt mit einer Meldung ab, statt den Server kommentarlos auf einem gescheiterten `build:web` neu zu starten. (#335)

## [3.4.1] – 2026-09-11

### Behoben
- **Sicherheit: `nodemailer` auf 9.1.1 angehoben** (vorher 9.0.5), was vier offene High-Advisories schließt: `resolveContent()` umging über die Legacy-Signatur `disableFileAccess`/`disableUrlAccess`, zwei Wege, die Empfänger-Domain-Prüfung auszuhebeln (IDN/Punycode sowie RFC-5322-Kommentare), und quadratische Laufzeit im `addressparser`. Reine Lockfile-Änderung — `^9.0.3` in `server/package.json` deckte die Version bereits ab.
- **Foto-Upload scheiterte dauerhaft mit einem nackten `401`, sobald ein veralteter API-Schlüssel im Browser lag.** `authHeaders()` bevorzugte `X-Api-Key` bedingungslos, und serverseitig betrachtet `requireApiKeyOrJwt` einen mitgeschickten Schlüssel als die einzige Anmeldung, die es prüft — ein toter Schlüssel konnte damit eine völlig gültige Server-Session überstimmen, ohne Fallback, ohne Refresh und ohne verwertbare Fehlermeldung. Ein 401 auf einen schlüssel-authentifizierten Foto-Request wird jetzt auf dem JWT wiederholt; der abgelehnte Schlüssel wird für den Rest der Sitzung nicht mehr gesendet, statt bei jedem Request eine Runde zu verbrennen. Bleibt nur der Schlüssel übrig, nennt die neue `ApiKeyInvalidError` den tatsächlichen Auslöser („API-Schlüssel ist ungültig — in den Einstellungen entfernen oder erneuern"), weil „Sitzung abgelaufen" hier aktiv in die Irre führt: ein Schlüssel läuft nicht ab und neu anmelden hilft nicht. Ursache für den veralteten Schlüssel war meist die Rotation im Admin-Panel — `rotateApiKey()` entfernt den Schlüssel jetzt aus dem `localStorage` des rotierenden Browsers, statt genau den liegen zu lassen, den der Aufruf gerade entwertet hat. Bewusst entfernt und nicht durch den frischen ersetzt: Rotation existiert, um eine geleakte Kopie zu verbrennen, und der Admin-Bereich ist ohnehin nur mit Admin-Session erreichbar. Ein abgelehnter Schlüssel wird außerdem in den Einstellungen als solcher ausgewiesen, und der zwischengespeicherte Foto-Token wird mitverworfen — er ist serverseitig mit dem Schlüssel signiert und hätte die Fotoanzeige sonst bis zu eine Stunde weiter auf 401 laufen lassen, obwohl der Upload schon wieder funktioniert. (#330)
...
### Hinzugefügt
- **Mehrere Finish-Werte pro Lack** (Web, Server, Android): `Polish.finish` ist jetzt eine Liste statt eines Einzelwerts — ein Lack kann z. B. „Top Coat" **und** „Glitter" gleichzeitig tragen. Die Finish-Auswahl ist auf allen Plattformen eine Mehrfachauswahl (Chip-Gruppe); Filter, Suche, Statistiken und Berichte zählen jeden Finish, den ein Lack trägt. Bestehende Daten werden beim ersten Start automatisch umgestellt — die Web-App zeigt dazu einen einmaligen Hinweis mit Rückgängig-Funktion, die Android-App legt vor der Room-Migration (v2 → v3) eine Sicherungskopie der Datenbank an. (#192, #197, #214)
- **Opt-in Zwei-Faktor-Authentifizierung (TOTP)** für Sync-Konten: Einrichtung unter Einstellungen → Sicherheit mit QR-Code und Recovery-Codes, zweistufiger Login, Sperre nach zu vielen Fehlversuchen. Vollständig freiwillig — bestehende Konten ohne 2FA verhalten sich unverändert. Die Android-App unterstützt den 2FA-Login noch nicht (#227). (#174)
- **Echtes Admin-Panel, alle Server-Einstellungen aus der Web-App erreichbar**: neue Rolle `admin`/`user` je Benutzerkonto, gated per JWT statt des rohen `X-Api-Key`. Der erste registrierte Account wird automatisch Admin (bei einem Neustart eines bestehenden Servers ebenso — `migrateFirstUserToAdmin()`); alternativ tauscht `POST /api/admin/bootstrap` den `X-Api-Key` einmalig gegen ein Admin-Konto ein. Neue Seite „Admin" (nur für Admins sichtbar) mit Benutzerverwaltung (anlegen/Rolle ändern/löschen), Server-Einstellungen (Registrierung, SMTP inkl. Testmail, KI-Anbieter inkl. Verbindungstest), API-Schlüssel-Rotation, Update-Installation und Audit-Log. Neuer Store `data/server_settings.json`: ein im Panel gesetzter Wert (SMTP, Registrierung) überschreibt die entsprechende Umgebungsvariable, die weiterhin als Fallback dient. (#173)
- **Web: Fehlergrenze (Error Boundary)** um jeden Tab und um die gesamte App. Ein einzelner beschädigter Datensatz zeigt jetzt eine Fallback-Ansicht mit „Neu laden" und „Lokale Daten zurücksetzen" (Letzteres hinter einer Rückfrage), statt die komplette Oberfläche verschwinden zu lassen. Die Navigation bleibt bedienbar, sodass Einstellungen und Export erreichbar bleiben. (#218)
- **Android: automatische Token-Erneuerung.** Läuft das Access-Token ab (Standard 7 Tage), erneuert die App es bei einem 401 selbstständig über `POST /api/auth/refresh` und wiederholt die Anfrage einmal. Vorher schlug der Sync ab diesem Zeitpunkt dauerhaft und wortlos fehl, bis man sich manuell neu anmeldete. (#220)
- **Android: `http://`-Server-URLs funktionieren wieder.** Ohne Netzwerk-Sicherheitskonfiguration blockierte Android jede Klartext-Verbindung, obwohl die App eine Warnung dafür anzeigte — das self-hosted LAN-Deployment war damit gar nicht nutzbar. Das Speichern einer `http://`-URL verlangt jetzt eine ausdrückliche Bestätigung. (#221)
- **Anleitungen unter `docs/`**: Installation, Erste Schritte, Sync, Datenspeicherung und Entwicklung sind eigene Seiten; die README ist von 233 auf 48 Zeilen geschrumpft und verweist nur noch. (#213)
...
- **Android: Beschriftungen der Navigationsleiste gekürzt** — „Wunschliste" → „Wünsche", „Tagebuch" → „Buch". Bei sechs Einträgen bleiben je rund 64dp, in denen die vollen Wörter mitten im Wort umbrachen. Die vollständigen Namen bleiben als Bedienhilfen-Beschriftung und in der Titelzeile jedes Bildschirms erhalten. (#240)
- **Android: Fotos sind vom Android-Backup ausgenommen**, die Sammlungsdatenbank bewusst nicht. Auto-Backup begrenzt eine App auf 25 MB; ein Fotoarchiv sprengt das und lässt das gesamte Backup fehlschlagen. Die Datenbank bleibt enthalten, weil sie ohne eingerichteten Sync beim Gerätewechsel sonst ersatzlos verloren wäre. (#223)
- **Android: Touch-Ziele auf 48dp vergrößert** — der „Foto entfernen"-Knopf und die Sterne-Bewertung waren nur so groß wie ihr Symbol. (#224)

Restart-pflichtige Einstellungen (`PORT`, `ALLOWED_ORIGIN`, `SERVICE_NAME`, `JWT_SECRET`, `DATA_DIR`, `NODE_ENV`, `APP_URL`) sind im Admin-Panel weiterhin nur informativ sichtbar, nicht live editierbar — siehe Begründung in der Server-Einstellungen-Sektion und in der PR-Beschreibung von #173.

### Behoben
- **Sicherheit: ein Token vor der 2FA-Prüfung öffnete Admin-Routen.** `requireAdmin` und zwei verwandte Guards prüften die Token-Version, aber nicht den Token-**Typ**. Ein `mfa`-Challenge-Token wird ausgestellt, *bevor* der zweite Faktor geprüft ist — damit galt ein halb abgeschlossener 2FA-Login als vollwertige Admin-Berechtigung, über `requireApiKeyOrAdminReconfirm` bis hin zu `POST /api/update/apply` (`git pull` + `npm install` auf dem Host). Gefunden beim Zusammenführen von #216 mit der 2FA-Arbeit. (#173/#174)
- **Sicherheit: `POST /api/admin/bootstrap` umging 2FA.** Für ein Konto mit aktiviertem TOTP wurden echte Tokens ausgestellt, wo `/api/auth/login` nur einen Challenge liefert. Jetzt liefert auch dieser Pfad den Challenge; die Rollen-Beförderung bleibt, der zweite Faktor schützt die Sitzung. Zusätzlich prüft der Pfad bei einem existierenden Benutzernamen jetzt das Passwort. (#173/#174)
- **Sicherheit: SMTP-Testversand konnte das gespeicherte Passwort preisgeben.** Der Host-Override war frei änderbar, während `pass` still auf den gespeicherten Wert zurückfiel — ein Admin-Token allein genügte, um den Server zu einer Anmeldung mit dem echten Passwort bei einem fremden Host zu bewegen. Host-/Benutzer-Override verlangt jetzt ein explizit mitgeschicktes Passwort. Ebenso wird ein TLS-Downgrade (`secure: false`) bei unverändertem Host ignoriert, solange kein Passwort mitgeschickt wird — sonst ginge das gespeicherte Passwort im Klartext über die Leitung. (Review-Funde zu #216)
- **Sicherheit: leerer `.api_key` wurde als gültiger Schlüssel akzeptiert.** `crypto.timingSafeEqual` hält zwei leere Puffer für gleich, wodurch eine zero-byte-Datei einen leeren `X-Api-Key`-Header akzeptiert hätte. Der Server startet in diesem Fall nicht mehr. (Review-Fund zu #216)
- **Datenverlust beim Sync verhindert:** `mergeData()` normalisiert `finish` jetzt zentral. Ein alter Client oder ein Cloud-Backup von vor der Umstellung konnte einen Einzelwert statt einer Liste liefern; per last-write-wins gewann dieser Eintrag, wurde gespeichert — und ließ danach jede Ansicht abstürzen, die über die Liste iteriert. Betraf Web-Sync, `POST /api/sync`, `POST /api/sync/push` und alle vier Cloud-Adapter. (#192)
- **Server: `POST /api/sync` und `/api/sync/push` prüfen die Nutzlast.** Vorher gab es nur eine Typannahme ohne Laufzeitprüfung: ein Feld als Zeichenkette statt als Liste wurde zeichenweise durchlaufen und als Datenmüll dauerhaft in die Sammlung geschrieben. Ungültige Nutzlasten werden jetzt mit 400 abgelehnt, bevor irgendetwas zusammengeführt wird. (#217)
- **Android: Tagebuch-Verknüpfungen überstehen Umbenennungen.** `PolishRef` trägt jetzt eine `id`, und die Auflösung greift zuerst darauf zurück (Name + Marke nur noch als Rückfall für Altdaten). Vorher verschwand ein Lack aus einem Eintrag, sobald er auf einem anderen Gerät umbenannt oder beim Import neu nummeriert wurde. (#219)
- **Android: Foto-Dateinamen aus dem Sync werden geprüft.** Namen aus fremden Daten flossen ungeprüft in die lokale Pfadbildung; ein Name mit `..` hätte aus dem Foto-Verzeichnis herausführen können. Noch nicht ausnutzbar, aber die Absicherung musste vor der Verdrahtung des Löschpfads stehen. (#222)
- **Android: verwaiste Fotos werden aufgeräumt.** Nach dem Löschen blieb die Bilddatei für immer liegen — `PhotoRepository.delete()` hatte keinen Aufrufer. Der Cleanup läuft nach erfolgreichem Sync und schont bewusst Tombstones (deren Fotos noch gebraucht werden) sowie Dateien der letzten 24 Stunden (ein gerade importiertes Foto in einem offenen Formular). (#226)
- **Android: Foto-Uploads erhalten die passende Dateiendung** statt immer `.jpg`, unabhängig vom tatsächlichen Bildtyp. (#225)

### Hinweise für Betreiber
- Beim ersten Start nach dem Update wird der zuerst registrierte Account automatisch zum Admin. Passiert nichts weiter, bleibt alles beim Alten — jeder bisherige `X-Api-Key`-Pfad funktioniert unverändert.
- Die Umstellung der Finish-Werte läuft automatisch. Ein Rückgängigmachen ist in der Web-App über den einmaligen Hinweis möglich; Android legt vor der Datenbank-Migration eine Sicherungskopie an.
