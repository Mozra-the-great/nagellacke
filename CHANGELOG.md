...
### Sicherheit
- **Android: HTTP-Logging-Interceptor nur noch im Debug-Build aktiv**: `ServerAdapter` und `NextcloudAdapter` fügten den OkHttp-`HttpLoggingInterceptor` (Level `BASIC`) bisher unbedingt hinzu, wodurch auch Release-Builds jede Sync-Anfrage/-Antwort ins Logcat schrieben — beim Nextcloud-Adapter inklusive Nutzername (Teil der WebDAV-Basis-URL). Jetzt hinter `BuildConfig.DEBUG` gated (`buildConfig = true` in `app/build.gradle.kts` aktiviert, damit das Feld überhaupt generiert wird). (#94)
