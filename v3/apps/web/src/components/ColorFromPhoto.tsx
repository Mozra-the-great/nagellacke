...
export default function ColorFromPhoto({ onColorPicked, onClose }: Props) {
  ...
  // Only ever mounted while open (the parent conditionally renders it), so
  // the trap is active for the component's whole lifetime — matches every
  // other overlay in the app (PolishFormModal, detail modals, ...) that
  // already uses this hook (#254).
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!imgUrl ? (
          <button type="button" className={styles.selectArea} onClick={() => fileInputRef.current?.click()}>
