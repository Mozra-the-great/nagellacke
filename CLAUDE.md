...
- Android cloud-sync OAuth (`OAuthClientIds` in `android/app/src/main/java/de/nagellacke/ui/settings/OAuthHelper.kt`): client IDs are `BuildConfig` fields sourced from the `GOOGLE_OAUTH_CLIENT_ID`/`MICROSOFT_OAUTH_CLIENT_ID`/`DROPBOX_OAUTH_APP_KEY` env vars at build time (`android/app/build.gradle.kts`), mirroring the signing-config env-var pattern — set them to your own app registrations. Without them the build falls back to unregistered placeholder values, which the respective OAuth server will reject. The Google Drive/OneDrive/Dropbox "connect" buttons in Settings are currently disabled (`enabled = false`, "in Kürze") pending that wiring, so this doesn't yet affect a reachable flow.
